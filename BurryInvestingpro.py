...
def is_non_traditional_asset(ticker: str, raw_info: Optional[Dict[str, Any]] = None) -> bool:
    ...
    if len(parts) == 2:
        ...
        if parts[0] in crypto_symbols and parts[1] in fiat_currencies:
            return True
    if raw_info:
...
def calculate_position_from_quantity(
    ...
    return {
        'Prezzo Attuale': current_price_in_user_cur if not np.isnan(current_price_native) else np.nan,
        ...
        'FX Native->User': get_fx_rate(native_cur, user_cur) if native_cur != user_cur else 1.0,
    }
